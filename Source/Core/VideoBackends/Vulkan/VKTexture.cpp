...
std::optional<AbstractTexture::RawTextureInfo> VKTexture::MapRegionImpl(u32 level, u32 x,
                                                                        u32 y, u32 width,
                                                                        u32 height)
{
  if (m_stagingTexture)
  ...
  m_stagingTexture->CopyFromImage(g_command_buffer_mgr->GetCurrentCommandBuffer(),
                                 m_texture->GetImage(), VK_IMAGE_ASPECT_COLOR_BIT, x, y,
  ...
  return AbstractTexture::RawTextureInfo{reinterpret_cast<u8*>(m_stagingTexture->GetMapPointer()),
                                         static_cast<u32>(m_stagingTexture->GetRowStride()),
                                         width, height};
}
